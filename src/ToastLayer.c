...
void toast_layer_hide(ToastLayer *this)
{
  if(this->is_visible == true)
  {
	...
	layer_mark_dirty(text_layer_get_layer(this->bg_layer));
	layer_mark_dirty(text_layer_get_layer(this->content_layer));
}
...
void toast_layer_show(ToastLayer *this, char *message, int duration, int offset)
{
	...
	// Auto hide
	this->toast_timer = app_timer_register(this->duration, timer_callback, this);
}
